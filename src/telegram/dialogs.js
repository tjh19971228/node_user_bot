...
class DialogManager {
  ...
  async checkDialogPermissions(dialog, isPassBroadcast = false) {
    try {
      // 跳过广播和论坛还有私聊
      if (
        dialog.isUser ||
        (!isPassBroadcast && dialog.entity.broadcast) ||
        dialog.entity.forum
      ) {
        return null;
      }
  ...
  async checkChannelPermissions(dialog) {
    ...
    const canPost =
      isAdmin || !participant.participant?.bannedRights?.sendMessages;
    if (canPost) {
      return {
        name: dialog.title,
        id: dialog.id,
  ...
  async getCanPostGroupAndChannel({ isAll = false, isPassBroadcast = false }) {
    const dialogs = isAll
      ? await this.getAllDialogs()
      : await this.getDialogs();

    for (const dialog of dialogs) {
      const dialogInfo = await this.checkDialogPermissions(
        dialog,
        isPassBroadcast
      );
      if (dialogInfo) {
